            ...
            if result:
                ...
                with col1:
                    if result['fake_probability'] > 0.70:
                        ...
                        st.write("Sahte kullanıcı davranışı tespit edildi.")

                    else:
                        st.metric("Tahmin", "Gerçek")
                        st.write("Doğal kullanıcı davranışı tespit edildi.")

                import plotly.graph_objects as go
                import time
                real_value = int(result['real_probability'] * 100)
                # Görsel gösterim
                placeholder = st.empty()
                # Animasyon efekti
                for i in range(0, real_value + 1):
                    fig = go.Figure(go.Indicator(
                        mode="gauge+number",
                        value=i,
                        title={'text': "Gerçek Olma İhtimali", 'font': {'size': 24}},
                        number={'suffix': "%", 'font': {'size': 32}},
                        gauge={
                            'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': "darkgray"},
                            'bar': {'color': "#aeaeae", 'thickness': 0.2},
                            'bgcolor': "white",
                            'borderwidth': 2,
                            'bordercolor': "gray",
                            'steps': [
                    ...
                    fig.update_layout(paper_bgcolor="#111111", font={'color': "white"})
...
with st.expander("Nasıl çalışır?"):
    st.write("""
    Bu uygulama, NLP, TF-IDF, Logistic Regression teknikleriyle çalışmaktadır. Ayrıntılı bilgi için **Nasıl Çalışır**a göz atabilirsiniz.
    """)